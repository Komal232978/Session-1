# write a program to input age of a user and let them know if they have diabete 
# risk or not. Use case is that a user with age more than 65 is risky else not
# risky

Age=55
if age>=65
print("This is risky")
else
print("not risky")

# Write a program to calculate the GST tsx of two products, apple and pie
# apple has 10% GST and pie has only 8%
# calculate total GST that need to be deducted from the bill
# apple gross sale is $75, pie gross is $100

apple = 75
pie = 100
gst_apple = 0.1*apple
gst_pioe = 0.08*pie

print(gst_apple)
print(gst_pie)

# Write a program to calculate the number of minis in the maxis.
# total maxis is 2.5*minis
# 1 minis is 5000
# calculate how much is 4.5 maxis


value for minis is 5000
maxis is 2.5*mins
 so 4.5 maxis is equal to 2.5*4.5*minis
display maxis

minis = 5000
maxis = 2.5*minis
print("the value for maxis is:", 4.5*maxis)
